...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    self.features = nn.Sequential(
      ...
      nn.ReLU(),
      nn.Dropout(p=0.5),
      nn.Conv2d(in_channels=10, out_channels=15, kernel_size=5,
        stride=1, padding=0, bias=False),
    ...
    self.classifier = nn.Conv2d(in_channels=15, out_channels=num_classes,
...
def create_part2_model(model, num_classes):
  ...
  m = nn.Linear(in_features=4096, out_features=15, bias=True)
  m.weight.data.normal_(0, 1)
  m.weight.data.mul_(1e-2)
  if m.bias is not None:
    # Initializing biases with zeros.
